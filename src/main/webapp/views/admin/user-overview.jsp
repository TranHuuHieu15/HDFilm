...
<head>
  ...
  <link rel="stylesheet" href="<c:url value='/templates/admin/plugins/summernote/summernote-bs4.min.css'/>">
	<!-- Sweet alert -->
	<%@include file="/common/cdnForAdmin.jsp"%>
</head>
	                  ...
	                  <tbody>
							<c:forEach var="user" items="${users }">
								<tr>
									...
									<td>${user.email }</td>
									<td>
										<a href="<c:url value='/admin/user?action=edit&username=${user.username }'/>" type="button" class="btn btn-block btn-warning btn-sm">Edit</a>
										<button type="button" onclick="deleteUser('${user.username}')" class="btn btn-block btn-danger btn-sm">Delete</button>
									</td>
								</tr>
							</c:forEach>
	                  </tbody>
...
<script>
  ...
  function deleteUser(username) {
	$.ajax({
		url: '/asmJava4/admin/user?action=delete&username=' + username
	}).then(function(data) {
		Swal.fire({
			  icon: 'success',
